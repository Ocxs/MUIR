...
def alignment(raw_data):
  data = {}
  for uid in raw_data:
    data[uid] = sorted(raw_data[uid], key=lambda x: x[2])
  return data

def main():
  data_dir = '/home/chen/Music'
  if 0:
    dataset = 'Video'
  else:
    dataset = 'Movie'
  ...
  for hist_length in [20]:
    print('emb_dim: {}, hist_length: {}'.format(emb_dim, hist_length))
    dropout_keep = 1.0
    #hist_length = 5
    raw_lin_pred = np.load(os.path.join(data_dir, 'LIN_{}_pred_{}.npy'.format(dataset, emb_dim))).item()
    raw_nin_pred = np.load(os.path.join(data_dir, 'NIN_{}_pred_{}.npy'.format(dataset, emb_dim))).item()
    raw_cin_pred = np.load(os.path.join(data_dir, 'CIN_{}_pred_{}.npy'.format(dataset, emb_dim))).item()
    #iin_pred = np.load(os.path.join(data_dir, 'IIN_{}_pred_{}_{}.npy'.format(dataset, emb_dim, att_dropout))).item()
    raw_iin_pred = np.load(os.path.join(data_dir, 'IIN_{}_pred_{}_{}_{}.npy'.format(dataset, emb_dim, hist_length, dropout_keep))).item()

    print('load data over')


    if dataset == 'Movie':
      lin_pred = alignment(raw_lin_pred)
      nin_pred = alignment(raw_nin_pred)
      cin_pred = alignment(raw_cin_pred)
      iin_pred = alignment(raw_iin_pred)
