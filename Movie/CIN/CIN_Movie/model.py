...
class Model(object):
  ...
  def build_model(self,
    ...
    with tf.variable_scope('feature_representation'):
      ...
      with tf.variable_scope('category_level_interest'):
        category_interest = tf.matmul(user_click_cate_distribution, self.category_embedding)
    ...
    if phase == 'train':
      emb_l2_loss = tf.nn.l2_loss(cate_emb) + tf.nn.l2_loss(self.category_embedding)
  ...
  def restore_user_click_cate_distribution(self, user_click_cate_distribution, sess):
    ...
    logging.info('load user click cate distribution into GPU memory successfully')
