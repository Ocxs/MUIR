...
class DataLoader(object):
  ...
  def load_map(self, data_dir):
    ...
    user_click_cate_distribution = [[] for _ in range(self.n_users)]
    with open(os.path.join(data_dir, 'user_click_cate_distribution.txt'), 'r') as reader:
      for line in reader:
        arr = line.strip('\n').split(':')
        click_distribution = [int(item) for item in arr[1].split(',')]
        if len(click_distribution) != self.n_cates:
          print(arr[0])
          break
        click_distribution = np.asarray(click_distribution)
        click_distribution = click_distribution / sum(click_distribution)
        user_click_cate_distribution[user_id_map[arr[0]]] = click_distribution

    self.user_click_cate_distribution = np.asarray(user_click_cate_distribution)

    return user_id_map, item_id_map, item_cate_map

  def del_temp(self):
    del self.visual_feature
    del self.user_click_cate_distribution