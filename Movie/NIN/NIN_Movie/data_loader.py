...
class DataLoader(object):
  def __init__(self, args):
    ...
    self.neighbor_sample = args.neighbor_sample
    ...
    self.item_cate_map, self.user_neighbor_map = self.load_map(os.path.join(args.data_dir, dataset))
  ...
  def get_batch_data(self, idx, mode='train'):
    ...
    neighbor_uids = [self.user_neighbor_map[uid] for uid in user_ids]
    return {
      'uid': user_ids,
      'iid': item_ids,
      'cid': cate_ids,
  ...
  def load_map(self, data_dir):
    ...
    user_neighbors_map = [[] for _ in range(self.n_users)]
    with open(os.path.join(data_dir, '{}_user_neighbors_map.txt'.format(self.neighbor_sample)), 'r') as reader:
      for line in reader:
        arr = line.strip('\n').split(':')
        neighbors = arr[1].split(',')
        user_neighbors_map[user_id_map[arr[0]]] = [user_id_map[item] for item in neighbors[:self.neighbor_length]]


    return user_id_map, item_id_map, item_cate_map, user_neighbors_map
