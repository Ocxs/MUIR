...
class Model(object):
  ...
  def build_model(self,
    ...
    with tf.variable_scope('feature_representation'):
      # user
      neighbor_user_embs = tf.nn.embedding_lookup(self.user_embedding, neighbor_uids)
      ...
      with tf.variable_scope('neighbor_interest_network'):
        avg_neighbor_emb = tf.reduce_mean(neighbor_user_embs, axis=1, keep_dims=True)
        neighbor_interest = neighbor_attention(neighbor_user_embs, item_emb, avg_neighbor_emb, 1.0,
                                               'relu', False, 1.0)


    with tf.variable_scope('aggregation'):
      if self.interaction == 'inner':
        fusion_input = neighbor_interest * item_emb
    ...
    if phase == 'train':
      emb_l2_loss = tf.nn.l2_loss(cate_emb) + tf.nn.l2_loss(neighbor_user_embs)
  ...
  def train(self, sess, data, lr):
    feed_dicts = {
      ...
      self.neighbor_uids_ph: data['nuid'],
      self.lr_ph: lr
    }
  ...
  def set_placeholder(self):
    ...
    self.neighbor_uids_ph = tf.placeholder(tf.int32, shape=(self.batch_size, self.neighbor_length))
  ...
  def init_embedding(self):
    ...
    # self.item_embedding = var_init('item_embedding', [self.n_items, dim],
    #                                tf.random_normal_initializer(mean=0.0, stddev=0.01))
    #
    # item_embedding2 = var_init('item_embedding2', [self.n_items, dim],
    #                            tf.random_normal_initializer(mean=0.0, stddev=0.01))
    # self.item_embedding2 = tf.concat([item_embedding2, tf.zeros((1, dim))], axis=0)
