...
class Model(object):
  ...
  def build_model(self,
    ...
    with tf.variable_scope('feature_representation'):
      ...
      hist_item_feat = tf.nn.embedding_lookup(self.train_visual_embedding, hist_iids)
      hist_item_emb = dense(hist_item_feat, self.emb_dim, ['w2'], keep_prob)
      mask = tf.sequence_mask(hist_length, self.hist_length)


    with tf.variable_scope('item_level_interest_network'):
      ...
      item_interest = item_interest_attention(hist_item_emb, hist_item_emb, item_emb, avg_hist_item_emb,
                                              mask, 1.0, 'sigmoid', True, 1.0)
    ...
    with tf.variable_scope('interaction'):
      if self.interaction == 'inner':
        logits = tf.reduce_sum(item_interest*item_emb, axis=1)
      else:
  ...
  def train(self, sess, data, lr):
    feed_dicts = {
      ...
      self.hist_iids_ph: data['hiids'],
      self.hist_length_ph: data['hl'],
  ...
  def set_placeholder(self):
    ...
    self.hist_iids_ph = tf.placeholder(tf.int32, shape=(self.batch_size, self.hist_length))
    self.hist_length_ph = tf.placeholder(tf.int32, shape=(self.batch_size,))
