...
class Model(object):
  ...
  def train_inference(self):
    ...
    inference_loss, l2_w_loss, l2_emb_loss, acc, _ = self.build_model(
      self.user_ids_ph,
      item_vec,
      self.cate_ids_ph,
      self.labels_ph,
  ...
  def build_model(self,
    ...
    with tf.variable_scope('interaction'):
      if self.interaction == 'inner':
        ...
        if self.interaction == 'product':
          fusion_input = user_emb * item_emb
        else:
    ...
    if phase == 'train':
      emb_l2_loss = tf.nn.l2_loss(user_emb) + tf.nn.l2_loss(cate_emb)
  ...
  def init_embedding(self):
    self.user_embedding = var_init('user_embedding', [self.n_users, self.emb_dim],
                                   tf.random_normal_initializer(mean=0.0, stddev=0.01, seed=self.seed+1))
