...
class Model(object):
  ...
  def build_model(self,
    ...
    with tf.variable_scope('feature_representation'):
      # user
      user_click_cate_distribution = tf.nn.embedding_lookup(self.user_click_cate_distribution, user_ids)
    ...
    with tf.variable_scope('category_level_interest_network'):
      cate_interest = tf.matmul(user_click_cate_distribution, self.category_embedding)
    ...
    with tf.variable_scope('interaction'):
      if self.interaction == 'inner':
        logits = tf.reduce_sum(cate_interest*item_emb, axis=1)
      else:
        if self.interaction == 'product':
          fusion_input = cate_interest * item_emb
        else:
    ...
    if phase == 'train':
      emb_l2_loss = tf.nn.l2_loss(self.category_embedding) + tf.nn.l2_loss(cate_emb)
  ...
  def restore_user_click_cate_distribution(self, user_click_cate_distribution, sess):
    distribution_ph = tf.placeholder(tf.float32, [self.n_users, self.n_cates])
    init = self.user_click_cate_distribution.assign(distribution_ph)
    sess.run(init, feed_dict={distribution_ph: user_click_cate_distribution})
    logging.info('load user behavior into GPU memory successfully')
  ...
  def init_embedding(self):
    ...
    self.user_click_cate_distribution = tf.Variable(tf.constant(0.0, shape=[self.n_users, self.n_cates]),
                                                    trainable=False, name='user_click_cate_distribution')

    self.category_embedding = var_init('category_embedding', [self.n_cates, self.emb_dim],
                                       tf.random_normal_initializer(mean=0.0, stddev=0.01, seed=self.seed+2))

    self.item_category_embedding = var_init('item_category_embedding', [self.n_cates, self.emb_dim],
                                       tf.random_normal_initializer(mean=0.0, stddev=0.01, seed=self.seed+4))
