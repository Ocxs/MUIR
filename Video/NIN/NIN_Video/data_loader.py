...
class DataLoader(object):
  ...
  def get_train_batch(self, idx):
    ...
    neighbor_user_ids = [self.neighbor_user_ids[uid][:self.neighbor_length] for uid in user_ids]
    return {
      'uid': user_ids,
      'iid': item_ids,
      'label': labels,
      'nuid': neighbor_user_ids,
      'cid': cate_ids
    }
  ...
  def load_data_from_disk(self):
    ...
    logging.info('load neighbor_user_ids.npy')
    path = os.path.join(self.data_dir, 'Video/neighbor_user_ids.npy')
    self.neighbor_user_ids = np.load(path)
